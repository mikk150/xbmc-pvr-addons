...
xbmc_codec_t g_audioCodec             = XBMC_INVALID_CODEC;
xbmc_codec_t g_videoCodec             = XBMC_INVALID_CODEC;
...
void ADDON_ReadSettings(void)
{
  ...
  if (XBMC->GetSetting("audio_codec_name", buffer))
    g_audioCodec = CODEC->GetCodecId(buffer);
  else
    g_audioCodec = CODEC->GetCodecId(DEFAULT_AUDIO_CODEC);

  /* read setting "video_codec_name" from settings.xml */
  if (XBMC->GetSetting("video_codec_name", buffer))
    g_videoCodec = CODEC->GetCodecId(buffer);
  else
    g_videoCodec = CODEC->GetCodecId(DEFAULT_VIDEO_CODEC);
...
ADDON_STATUS ADDON_SetSetting(const char *settingName, const void *settingValue)
{
  ...
  else if (str == "video_codec_name")
  {
    ...
    if (tmp_strCodecname != (const char*) settingValue)
    {
      ...
      g_videoCodec = CODEC->GetCodecId((const char*) settingValue);
      return ADDON_STATUS_OK;
    }
  }
  else if (str == "audio_codec_name")
  {
    ...
    if (tmp_strCodecname != (const char*) settingValue)
    {
      ...
      g_audioCodec = CODEC->GetCodecId((const char*) settingValue);
      return ADDON_STATUS_OK;
    }
  }
