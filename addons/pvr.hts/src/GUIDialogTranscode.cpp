...
bool CGUIDialogTranscode::OnInit()
{
  ...
  {
    if (CODEC->CodecIsAudio(m_codecs.at(iPtr).codec_type))
    {
      m_spinAudioCodec->AddLabel(m_codecs.at(iPtr).name, iPtr);
      if (m_codecs.at(iPtr).codec_id == g_audioCodec.codec_id)
        iSelectedAudio = iPtr;
    }
    else if (CODEC->CodecIsVideo(m_codecs.at(iPtr).codec_type))
    {
      m_spinVideoCodec->AddLabel(m_codecs.at(iPtr).name, iPtr);
      if (m_codecs.at(iPtr).codec_id == g_videoCodec.codec_id)
        iSelectedVideo = iPtr;
    }
  }
