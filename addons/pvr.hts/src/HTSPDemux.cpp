...
void CHTSPDemux::ParseSubscriptionStart(htsmsg_t *m)
{
  ...
  HTSMSG_FOREACH(f, streams)
  {
    ...
    xbmc_codec_t codecId = CODEC->GetCodecId(type);
    if (!CODEC->CodecIsUnknown(codecId.codec_type))
    {
      newStream.iCodecType = codecId.codec_type;
      newStream.iCodecId   = codecId.codec_id;

      if (CODEC->CodecIsSubtitle(codecId.codec_type))
      {
    ...
    if (bValidStream)
    {
      XBMC->Log(LOG_DEBUG, "%s - id: %d, type: %s, codec: %u", __FUNCTION__, index, type, codecId.codec_id);

      newStream.iPhysicalId  = index;
      if (CODEC->CodecIsAudio(codecId.codec_type))
      {
        HTSPSetDemuxStreamInfoAudio(newStream, sub);
        HTSPSetDemuxStreamInfoLanguage(newStream, sub);
      }
      else if (CODEC->CodecIsVideo(codecId.codec_type))
...
bool CHTSPDemux::SendSubscribe(int subscription, int channel)
{
  ...
  if(g_bTranscode)
  {
    audioCodec = g_audioCodec.name ? g_audioCodec.name : "UNKNOWN";
    videoCodec = g_videoCodec.name ? g_videoCodec.name : "UNKNOWN";
